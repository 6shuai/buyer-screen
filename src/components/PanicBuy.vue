<template>
    ...
    <div 
        ...
        :class="{ sell_out_crad: gameState == 4 }"
    >
        <div class="data" v-if="gameState == 4">已售罄</div>
        <div class="data" v-else>实时价格: <span class="price_int">￥{{ realTimePrice.int }}</span><span class="price_decimals">{{ realTimePrice.decimals }}</span></div>
    </div>
</template>
