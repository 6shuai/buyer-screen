<template>
    ...
    <div class="goods_detail">
        ...
        <div class="goods_detail_right">
            <p class="goods_name text_overflow text_medium">{{ data.goodsName }}<span class="goods_desc">{{ data.goodsDescription }}</span></p>
            <p class="goods_count text_medium">宝贝库存: 10</p>
            <p class="goods_del_price text_medium">￥{{ priceFormat(data.marketValue).int }}<span class="decimals">{{ priceFormat(data.marketValue).decimals }}</span></p>
        </div>
    </div>
